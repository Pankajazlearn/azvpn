data "azurerm_subnet" "subnet1" { #--We need to look this up as as list as we need to get the ID of the Subnet
    name                 = "GatewaySubnet"
...
resource "azurerm_local_network_gateway" "madcaplaughs" {
    ...
    resource_group_name = var.resource_group
    gateway_address     = "90.220.50.60" #--Your local device public IP here
...
resource "azurerm_virtual_network_gateway" "tinfoilvpn" {
    ...
    ip_configuration {
        ...
        subnet_id                     =   data.azurerm_subnet.subnet1.0.id#--There's that ID we needed, for the Transport Subnet
    }
}
